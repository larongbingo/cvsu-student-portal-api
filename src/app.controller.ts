...
export class AppController {
  ...
  public async GetGrades(@Query("sessionId") sessionId: string) {
    const grades = await this.parsedStudentPortalService.GetParsedGrades(
      sessionId,
    );
    return { grades };
  }
  ...
  public async GetProfiles(@Query("sessionId") sessionId: string) {
    const profile = await this.parsedStudentPortalService.GetParsedProfile(
      sessionId,
    );
    return { profile };
  }
  ...
  public async GetBalance(@Query("sessionId") sessionId: string) {
    const balance = await this.parsedStudentPortalService.GetParsedAccountBalance(
      sessionId,
    );
    return { balance };
  }
  ...
  public async GetEnrolledSubject(@Query("sessionId") sessionId: string) {
    const enrolledSubjects = await this.parsedStudentPortalService.GetParsedEnrolledSubjects(
      sessionId,
    );
  ...
  public async GetEnrolledStudentsInSchedule(
    @Query("scheduleCode") scheduleCode: number,
  ) {
    const students = await this.parsedPublicPortalService.GetParsedStudentsEnrolledInSubject(
      scheduleCode,
    );
    return { students };
  }
  ...
  public async GetSubjects(
    @Query("startingId") startingId: number,
    @Query("length") length: number,
  ) {
    const subjects = await this.publicPortalService.GetListOfSubjects(
      startingId,
      length,
    );
